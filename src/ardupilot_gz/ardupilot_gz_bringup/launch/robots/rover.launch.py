...
def generate_launch_description():
    ...
    sitl_dds = IncludeLaunchDescription(
        ...
        launch_arguments={
            "home": "-38.70198362535839,177.90832136419021,0,0",
            "transport": "udp4",
            "port": "2019",
            "command": "ardurover",
            "synthetic_clock": "True",
    ...
    sdf_file = os.path.join(
        pkg_ardupilot_sitl_models, "models", "rover_with_lidar", "model.sdf"
    )
    with open(sdf_file, "r") as infp:
        ...
        robot_desc = robot_desc.replace(                     #??????????????????????????
            "model://rover_with_lidar",
            "package://ardupilot_sitl_models/models/rover_with_lidar")
        ...
        parameters=[
            {
                "config_file": os.path.join(
                    pkg_project_bringup, "config", "rover_bridge.yaml"
                ),
